...
loadedModel = keras.models.load_model('Lenet_Mnist_Run1')
loadedModel.predict(tf.ones((1, 28, 28, 1))) # hack to for the model to properly instantiated
...
bias_NetTrim, U_dict_NetTrim = getLayerApprox(model=loadedModel, layer_idx=5, N=N_app,
                                      ...
                                      method = 'NetTrim')
np.save('approx_matrices/net_trim_layer_1_Run1_N_128.npy', U_dict_NetTrim)
...
np.save('approx_matrices/DLR_layer_1_Run1_N_128.npy', U_dict_DLR)
...
bias_NetTrim, U_dict_NetTrim = getLayerApprox(model=loadedModel, layer_idx=6, N=N_app,
                                              epsilon_list=epsilon_list, inputs=train_images,
                                              method = 'NetTrim')
np.save('approx_matrices/net_trim_layer_2_Run1_N_128.npy', U_dict_NetTrim)
...
bias_DLR, U_dict_DLR = getLayerApprox(model=loadedModel, layer_idx=6, N=N_app, 
                                      epsilon_list=epsilon_list, inputs=train_images, 
                                      method = 'DLR', verbose=True)
np.save('approx_matrices/DLR_layer_2_Run1_N_128.npy', U_dict_DLR)